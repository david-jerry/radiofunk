...
class RadioQuerySet(QuerySet):
    ...
    def country(self, query=None):
        qs = self
        if query is not None:
            lookup = Q(country__iso__icontains=query)
            qs = qs.active().filter(lookup).distinct()

    def closest(self, dist=None, cur_loc=None):
        qs = self
        if cur_loc is not None and dist is not None:
            qs = qs.active().filter(location__dwithin = (cur_loc, dist)).annotate(distance=GeometryDistance("location", cur_loc)).order_by("distance").distinct()
        return qs
